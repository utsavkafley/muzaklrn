    ...
    >
      <svg width={W} height={H}>
        <text x={W / 2} y={13} textAnchor="middle" fontSize={12} fontWeight={700}
          className={active ? "fill-amber-300" : "fill-neutral-200"}>
          {chordName(chord)}
        </text>
        {/* nut or fret label */}
        {lo === 1 ? (
          <line x1={gridL} y1={gridT} x2={gridL + gridW} y2={gridT} stroke="#e5e5e5" strokeWidth={3} />
        ) : (
          <text x={gridL - 6} y={rowY(0) + 14} textAnchor="end" fontSize={9} className="fill-neutral-400">
            {lo}fr
          </text>
        )}
        {Array.from({ length: 5 }, (_, r) => (
          <line key={r} x1={gridL} y1={rowY(r)} x2={gridL + gridW} y2={rowY(r)} stroke="#525252" strokeWidth={1} />
        ))}
        {Array.from({ length: 6 }, (_, s) => (
          <line key={s} x1={colX(s)} y1={gridT} x2={colX(s)} y2={gridT + gridH} stroke="#525252" strokeWidth={1} />
        ))}
        {frets.map((f, s) => {
          if (f < 0)
            return <text key={s} x={colX(s)} y={gridT - 4} textAnchor="middle" fontSize={9} className="fill-neutral-500">✕</text>;
          if (f === 0)
            return <circle key={s} cx={colX(s)} cy={gridT - 7} r={3.2} fill="none" stroke="#a3a3a3" strokeWidth={1.2} />;
          const row = f - lo; // 0-based row within window
          return (
            <circle key={s} cx={colX(s)} cy={rowY(row) + gridH / 8} r={5.5}
              fill={active ? "#fbbf24" : "#e5e5e5"} />
          );
        })}
