...
export default function ListenPage() {
  ...
  return (
    <div className="space-y-5">
      <header>
        <h1 className="font-[family-name:var(--font-caveat)] text-4xl text-amber-400">Listen</h1>
      ...
      {!connected && (
        <div className="rounded-2xl border border-emerald-800/60 bg-emerald-950/30 p-4 text-sm">
          {configured ? (
            <>
              <p className="text-neutral-200">Connect Spotify to pull your real listening history.</p>
            ...
            <>
              <p className="font-bold text-emerald-300">Demo mode</p>
              <p className="mt-1 text-neutral-300">
                To go live: create an app at <span className="text-emerald-300">developer.spotify.com/dashboard</span>,
                add <code className="rounded bg-neutral-800 px-1">{typeof window !== "undefined" ? `${window.location.origin}/listen` : "…/listen"}</code> as
                a Redirect URI, then set <code className="rounded bg-neutral-800 px-1">NEXT_PUBLIC_SPOTIFY_CLIENT_ID</code> in
                Vercel (or <code className="rounded bg-neutral-800 px-1">.env.local</code>). No secret needed — it uses PKCE.
              </p>
            </>
          )}
        </div>
          ...
          {rows.map((row, i) => {
            ...
            return (
              <details key={i} onToggle={(e) => (e.target as HTMLDetailsElement).open && loadFact(row)}
                className="group rounded-2xl border border-neutral-800 bg-neutral-900/50 open:border-amber-400/40">
                <summary className="flex cursor-pointer list-none items-center gap-3 p-3">
                  {row.image ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={row.image} alt="" className="h-11 w-11 rounded-lg object-cover" />
                  ) : (
                    <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-neutral-800 text-lg">♪</div>
                  )}
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-bold text-neutral-100">{row.song}</p>
                    <p className="truncate text-sm text-neutral-400">{row.artist}</p>
                  </div>
                  <span className="text-neutral-600 transition-transform group-open:rotate-90">›</span>
                </summary>
                <div className="space-y-3 px-3 pb-3">
                  {fact && <p className="text-sm text-neutral-300">{fact}</p>}
                  <div className="flex flex-wrap gap-2 text-sm">
                    ...
                    <a href={ytSearchUrl(`${row.artist} ${row.song}`)} target="_blank" rel="noreferrer"
                      className="rounded-full border border-neutral-700 px-4 py-2 text-neutral-300 hover:border-neutral-500">
                      Lesson on YouTube ↗
                    </a>
                    ...
                    {row.url && (
                      <a href={row.url} target="_blank" rel="noreferrer"
                        className="rounded-full border border-emerald-800 px-4 py-2 text-emerald-400 hover:border-emerald-600">
                        Open in Spotify ↗
                      </a>
                    )}
                  </div>
